#!/usr/bin/env python3
"""
Enhanced Backend Readiness Smoke Test for Production Ingress
Testing at: https://aurum-overflow-fix.emergent.host/api

This version includes better error handling and diagnostics for authentication issues.
"""
...
class EnhancedBackendSmokeTest:
    def __init__(self):
    ...
    def log_result(self, test_name, success, details, response_time=None):
        ...
        if not success or (details and "successful" not in details.lower()):
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                self.log_result(
                    "User registration", 
                    True, 
                    f"Registration blocked ({response.status_code}) - continuing as instructed",
                    response_time
                )
            else:
                self.log_result(
                    ...
                    try:
                        ...
                        if missing_fields:
                            ...
                            self.log_result(
                                "User profile retrieval", 
                                True, 
                                f"Profile retrieved successfully: id={data.get('id', 'N/A')[:8]}..., email={data.get('email', 'N/A')}",
                                response_time
                            )
                    except json.JSONDecodeError:
                        self.log_result(
    ...
    def test_minimal_crud_operations(self):
        ...
        if not self.access_token:
            self.log_result("CRUD operations", False, "No valid access token available for CRUD tests")
            return
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                try:
                    ...
                    if 'id' in data:
                        ...
                        self.log_result(
                            "Pillar creation", 
                            True, 
                            f"Pillar created successfully: id={self.created_pillar_id[:8]}...",
                            response_time
                        )
                    else:
                        self.log_result(
                ...
                self.log_result(
                    "Pillar creation", 
                    False, 
                    f"Authentication failed - token may be invalid or expired",
                    response_time
                )
        ...
        if self.access_token:  # Only proceed if we still have a valid token
            try:
                start_time = time.time()
            ...
            self.log_result("Pillar retrieval", False, "No valid access token available")
        
        # Step 4c: DELETE /api/pillars/{id} (delete)
        if self.created_pillar_id and self.access_token:
            try:
                start_time = time.time()
            ...
            reason = "No pillar ID available" if not self.created_pillar_id else "No valid access token"
            self.log_result("Pillar deletion", False, f"{reason} for deletion")
            ...
            try:
                ...
                if missing_headers:
                    ...
                    self.log_result(
                        f"Security headers ({endpoint.split('/')[-1]})", 
                        True, 
                        f"All required headers present",
                        response_time
                    )
    ...
    def run_all_tests(self):
        """Run all smoke tests"""
        print(f"🚀 Starting Enhanced Backend Readiness Smoke Test")
        ...
        for result in self.test_results:
            if not result['success']:
                if any(keyword in result['test'].lower() for keyword in ['auth', 'login', 'profile', 'security']):
                    critical_failures.append(result)
                else:
                    minor_failures.append(result)
        
        if len(critical_failures) == 0:
            print("🎉 CORE SYSTEMS OPERATIONAL - Backend authentication and security are working!")
            if len(minor_failures) > 0:
                print("⚠️  Minor issues detected in CRUD operations - may not block UI E2E testing")
        else:
            print("🚨 CRITICAL ISSUES DETECTED - Backend not ready for UI E2E testing")
        
        if failed_tests > 0:
            print("\nFailed Tests:")
            for result in self.test_results:
                if not result['success']:
                    criticality = "🚨 CRITICAL" if result in critical_failures else "⚠️  MINOR"
                    print(f"   {criticality}: {result['test']}")
...
if __name__ == "__main__":
    test_runner = EnhancedBackendSmokeTest()
