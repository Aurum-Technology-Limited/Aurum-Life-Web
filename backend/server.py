...
if frontend_origins_env:
    ...
    ALLOWED_ORIGINS = [
        'https://productivity-hub-23.preview.emergentagent.com',
        'http://localhost:3000',
        'http://127.0.0.1:3000',
        'https://productivity-hub-23.preview.emergentagent.com'
    ]
...
async def get_task(task_id: uuid.UUID, current_user: User = Depends(get_current_active_user_hybrid)):
    try:
        user_id = str(current_user.id)
        supabase = get_supabase_client()
        resp = supabase.table('tasks').select('*').eq('id', str(task_id)).eq('user_id', user_id).single().execute()
        if not resp.data:
