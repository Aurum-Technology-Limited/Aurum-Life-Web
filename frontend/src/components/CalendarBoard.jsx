          ...
          <div key={d.toISOString()} className="min-h-[90px] border border-gray-800 rounded p-1">
            ...
            <div className="space-y-0.5">
              {dayTasks.slice(0, 2).map((t) => (
                <div key={t.id} className="text-[10px] truncate cursor-pointer hover:underline" onClick={() => onOpen(t)}>• {t.name}</div>
              ))}
...
const CalendarBoard = () => {
  ...
  const loadProjects = useCallback(async () => {
    try { const resp = await projectsAPI.getProjects(); setProjects(Array.isArray(resp.data) ? resp.data : []); }
    catch { setProjects([]); }
  }, []);

  const load = useCallback(async () => {
    try { setLoading(true); const resp = await tasksAPI.getTasks(); setTasks(Array.isArray(resp.data) ? resp.data : []); }
    catch { setTasks([]); }
  ...
  const onDropTask = useCallback(async (taskId, date) => {
    try { await tasksAPI.updateTask(taskId, { due_date: date.toISOString() }); await load(); }
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              <button
                ...
                onKeyDown={(e) => { if (showFilter && e.key === 'Escape') setShowFilter(false); }}
                aria-haspopup="menu"
              ...
              {showFilter && (
                <div
                  ref={filterMenuRef}
                  tabIndex={0}
                  className="absolute right-0 top-8 bg-gray-900 border border-gray-700 rounded p-3 z-50 w-64 outline-none"
                  onKeyDown={(e) => {
                    ...
                    if (e.key === 'ArrowDown') { e.preventDefault(); setFilterActiveIndex((i) => Math.min(i + 1, max)); }
                      ...
                      const p = projects[filterActiveIndex]; if (p) {
